...
def compRelEffCurve(
    shape=np.linspace(1, 2, 10, endpoint=False),
    n=np.linspace(2, 2000, 50).astype(np.int),
    kappa=0.01,
    k=20,
    c=100,
    verbose=False,
...
def compRelEffVarC(kappa=0.005):
    ...
    return [
        compRelEffCurve(shape=np.linspace(1, 3, 10, endpoint=False), c=i, kappa=kappa)
        for i in C
    ]
...
def processRatio(ratio):
    def rQuantile(x, q=[0.25, 0.5, 0.75]):
